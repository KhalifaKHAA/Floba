...
const handler = async (m, { text, conn, participants, usedPrefix, command }) => {
  if (!text) {
    ...
    if (action === 'اسحب') {
      ...
      const cap2 = `تسجيل سحب المجموعة بواسطه مطورى \nمطورى : @${m.sender.split('@')[0]} 💡\nشكرا لاستضافتكم لي \n\n> اطلب من مطورى ردها مجددا اذا كنت تريد .`;
      await conn.sendMessage(idgru, { text: cap2, mentions: [m.sender] }, { quoted: m });
      await conn.sendMessage(m.chat, { text: '*تم سحب المجموعة بنجاح ي مطورى *', mentions: [m.sender] }, { quoted: m });
      ...
      await conn.sendMessage(m.chat, { text: '*تم تصفية المجموعة بنجاح ي مطورى *', mentions: [m.sender] }, { quoted: m });

    } else if (action === 'مغادرة') {
      const cap2 = `تسجيل خروجي بواسطه مطورى \nمطورى : @${m.sender.split('@')[0]} \nشكرا لاستضافتكم لي \n\n> اطلب من مطورى اضافتي مجددا اذا كنت تريد .`;
      await conn.sendMessage(idgru, { text: cap2, mentions: [m.sender] }, { quoted: m });
      await conn.groupLeave(idgru);
      await conn.sendMessage(m.chat, { text: '*تمت العملية بنجاح ي مطورى *', mentions: [m.sender] }, { quoted: m });
    }
  }
};
